...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        for ch in word:
            if ch in self.handDict:
                if self.handDict[ch] == 0:
                    raise ValueError('letter ' + str(ch) + ' not in hand')
                else:
                    self.handDict[ch] -= 1
            else:
                raise ValueError('letter ' + str(ch) + ' not in hand')
    ...
    def __eq__(self, other):
        ...
        """
        for k in self.handDict:
            if not self.handDict[k] == other.handDict[k]:
                return False
        return True
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        if self.points > other.points:
            ...
            raise ValueError("couldn't compare points")
    def __str__(self):
        """
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist:
            ...
            #can Player spell it?
            if self.getHand().containsletter(word):
                    #how many points?
                points += SCRABBLE_LETTER_VALUES[ch]
                GOT THIS FAR
                if len(wordpoints) == 0:
                    wordpoints.append((word,points))
                else:
                    for i in range(len(wordpoints)):
                        if points <= wordpoints[i][1]:
                            wordpoints.insert(i+1,(word,points))
                print wordpoints
...
#not in pset, but here to test
def test():
    h = Hand(10)
    print h

    P = ComputerPlayer(1,h)
    words = ['badf','dsfers','cadsf','csadffg']
    print 'testing', words
    P.pickBestWord(words)
